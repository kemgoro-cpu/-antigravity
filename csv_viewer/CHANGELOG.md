...
## 2026-09-09: 複数ログ比較ワークスペース

- 3・4ファイルで元のMain/Sub切替、相対時間差、差分の数値、上下表示、比較結果と再起動復元をブラウザ検証。復元設定を最初のファイルだけで消費する不具合を修正し、全ファイルの読込後に基準・時間差・選択を復元。
- 準備 → 時間合わせ → 比較結果 → メモ・保存を右パネルでつなぎ、表示名・基準選択・ファイル別の信号対応を一括操作。
- 波形を重ね合わせ／上下比較／差分へ切替。全体ナビゲーター、直接の時刻入力、区間操作バーを追加。
- 自動合わせは候補の波形・RMSE・重なり率を表示してから適用。2点クリック／数値指定・元の時刻表示にも対応。
- 元データで区間差、許容差超過、Bit不一致時間と最初の切替時刻を集計。単位不一致、欠損、重なり不足を明示。
- チャンネル絞り込み・検索別名・キーボード選択・ドラッグ重ね合わせ／独立段挿入を追加。
- メモに画像・解析条件・区間数値を保存し、HTMLレポートへ収録。比較テンプレートの保存／適用／JSON入出力を追加。
- 設定v6へ移行。比較設定は既存の永続化・Undoに統合。テンプレートは前回のシフトを再利用せず、新ログで再計算する。
- 描画されている系列からホバー／統計／カーソル計測を集計し、差分・上下比較・元時刻表示との不一致を解消。
- notMerge再描画／プレビュー破棄前にhideTipを発行し、選択区間を再描画した際のECharts tooltip競合を防止。
- 既存のNEDC MT/AT・走行モードの変更を保持。

検証：単体テスト8本、Edgeでのfile://ブラウザテスト2本。詳細な操作・制限・実行方法はREADMEの比較ワークスペース節を参照。

開発上の注意：比較用の表示グループID（上下比較）は元のchartGroupsのIDと一致しないため、
軸・値表示では描画スナップショットを使う。配置編集は重ね合わせ表示で行う。
比較テンプレート／メモの適用はapp.jsから渡す`applyComparison`で一括処理し、途中状態をUndoに記録しない。

## 2026-07-27: NEDCのAT版モードを追加し、トランスミッション(MT/AT)を自動判定

内蔵していたNEDCの目標車速トレースは **MT（マニュアル）版**だった。加速の途中に
15 / 35 / 50 km/h ちょうどで2秒間の定速保持が15箇所あり、これは法規のサイクル表にある
変速（クラッチ切断）区間。AT/CVT車の試験ではこのパターンを使わないため、AT車のログを
評価すると加速区間で実測と目標がずれ、指標が実力より悪く出ていた。

### 効果（同一のAT追従データでの実測値）

| 目標 | RMSSE | DR（距離偏差） |
|---|---|---|
| MT版で評価 | 1.299 km/h | −0.822% |
| AT版（自動判別） | 0.382 km/h | +0.009% |

RMSSEが3.4倍に膨らみ、実際は0%の距離偏差が−0.82%と出ていた。
なお IWR / ASCR はMT/ATで変化しない（正の慣性仕事も絶対速度変化も、同じ区間で
0→32km/h まで単調に上がる限り経路によらず同じ値になるため）。**判別に効くのは RMSSE と DR**。

### 変更内容

1. **AT版トレースの導出** (`drive-cycles-data.js`): `automaticVariant()` を追加。
   静的データを複製せずMT版から導出する（7KBの重複を持つとMT版と静かに食い違うため）。
   根拠は UN R83 を採用した MoRTH/CMVR TAP-115/116 (Issue 4) Chapter 3 §2.3.3:
   > "the gear change points ... do not apply: acceleration must continue throughout the
   > period represented by the straight line connecting the end of each period of idling
   > with the beginning of the next following period of steady speed."

   つまり変速のための保持を消すだけでなく、**アイドリング終了点から次の定速走行開始点までを
   1本の直線に置き換える**。適用結果は9箇所（0→32 km/h が12秒 0.741 m/s²、0→50 km/h が
   26秒 0.534 m/s²、0→70 km/h が41秒 0.474 m/s²）。総距離 11.0132 → 10.9217 km。
   キーは `nedc_at`（`DERIVED` マップ経由の遅延評価）。

2. **レジストリのMT/AT分割** (`drive-index-utils.js`): `nedc` → `nedc_mt` / `nedc_at`。
   `transmission`（'MT'/'AT'）と `variantGroup`（'nedc'）を追加。
   **省略＝MT/ATを区別しないモード**を意味し、WLTC・MDCは両方とも持たない（後述）。
   `LEGACY_CYCLE_ID` に `nedc: 'nedc_mt'` を追加（分割前のトレースはMT版だったため）。

3. **変種判別** (`drive-index-utils.js`): `pickTransmissionVariant()` を追加。
   MT版とAT版は総時間も最高車速も同じで、**波形全体のRMS差も1.148 km/h**しかなく
   実車の追従誤差に埋もれる。そこで**変種同士で差が出る184サンプルだけ**を取り出して比較する
   （同区間に限ればRMS差2.907 km/h）。1位と2位の差が0.5 km/h未満なら決め手なしとして
   `null` を返し、呼び出し側がMT版（レジストリ先頭）へ倒せるようにした。

4. **判別フローへの組み込み** (`app.js`): `refineTransmissionVariant()` を追加し、
   モード確定後に `variantGroup` を持つモードだけ変種を選び直す。
   実測のどこからサイクルが始まるかは既存の `alignActualToCycle` で求める。

### WLTC / MDC は MT/AT を区別しない（変更なし）

MT/AT の区別が要るのは NEDC だけ。WLTP は目標車速トレースを1本だけ定義し、MT車の変速段は
車両ごとにギヤシフト計算手順で別途求める方式なので、目標車速はMT/ATで変わらない。
データ上も、WLTC/MDCの平坦点は 46.0 / 47.4 / 65.3 / 58.8 km/h など不規則な値で数箇所しかなく、
NEDCが 15 / 35 / 50 km/h ちょうどで15箇所あるのとは性質が違う（実走行由来のトレースに
たまたま現れる平坦点）。よって WLTC・MDC のレジストリ項目は変更せず、
チャンネル名も `@MDC` / `@WLTC3b_3` のまま。変種判別も走らない。

### 未確認事項（引き継ぎ注意）

導出したAT版は**規定の条文を実装した結果であって、公式配布のデータファイルではない**。
条文の出典はUN R83を採用したインド規格（ARAI）で、R83本文そのものは入手できなかった
（unece.org は403、EUR-LexはPDF解析不可）。試験室で実際に使っているAT用パターンと
突き合わせて確認すること。食い違う場合は導出をやめて実データを取り込む形に切り替える。

### 検証

- `npm test` 全7本パス。AT版の導出（9箇所の直線が厳密に等間隔・変速区間ゼロ・総距離）、
  変種判別（ノイズ付き・時間シフト付き・中間波形でnull）のテストを追加。
- ブラウザ実機: MT追従データ→`NEDC (MT)`＋`@NEDC_MT` / AT追従データ→`NEDC (AT)`＋`@NEDC_AT` /
  同梱 `NEDC_sample_A.trn`→`NEDC (MT)` / MDC・WLTCは名前もチャンネル名も変化なし /
  モード一覧にMT・AT両方が並ぶ / コンソールエラーなし。

## 2026-07-27: 判別した走行モードの目標車速を `@MDC` 等のチャンネルとして表示可能に

Driving Index はモードを判別して指標を数値で出せるが、**目標車速の波形そのものをグラフに
重ねる手段が無かった**（「どこでどうズレたか」が見えない）。判別結果から目標車速チャンネルを
自動生成し、チャンネル一覧のクリックだけでグラフに出せるようにした。

### 変更内容

1. **モードの短縮名** (`drive-index-utils.js`): `CYCLE_REGISTRY` の各エントリに `shortName` を追加
   （`NEDC` / `WLTC3b_4` / `WLTC3b_3` / `WLTC3a_4` / `WLTC3a_3` / `MDC`）。
   表示名 `name` は「WLTC Class 3b (3フェーズ)」のように空白・括弧を含み Custom RAM の式に
   書けないため別に持つ。

2. **`cycleChannelName(mode)` を追加** (`drive-index-utils.js`): モード定義から `@MDC` 形式の
   チャンネル名を作る純粋関数。ユーザー定義モード（`customModes`）は任意の名前を付けられるので、
   式パーサ（`tokenizeExpr`）が識別子の区切りに使う文字 `[\s+\-*/()^,<>=!&|]` を除去する。
   除去して空になる名前は `id` にフォールバック。

3. **`syncCycleTraceColumns()` を追加** (`app.js`): 判別結果から各ファイルに合成列を作る。
   - `state.customRAMs` には登録せず**毎回作り直す**。目標トレースは式ではなく判別結果から
     導かれる派生データなので、状態を判別結果ひとつに従わせるほうが破綻しない
     （モード切替時に「`@MDC` という名前なのに中身はWLTC」になる問題を構造的に回避）。
   - 実測時間軸への写像は `computeDriveIndex` が既に求めている `entry.align`（start/end）を
     再利用するため、手動整合・自動整合・区間スケール補正がそのまま効く。
   - **サイクル窓の外は NaN**（前後の余分データ区間には線を引かない）。
   - メイン・サブそれぞれ自分の整合位置で作られる。
   - 列は `isCustom: true`（遅延パースとBit判定の対象外）＋ `isCycleTrace: true`（この関数だけが
     管理する目印）。

4. **`recomputeCustomRAMs` から保護** (`app.js`): 同関数は `!c.isCustom` で全合成列を捨てるため、
   `!c.isCustom || c.isCycleTrace` に変更。

5. **呼び出し** (`app.js`): `computeDriveIndex` の末尾と早期リターン経路の両方で
   `applyCycleTraceColumns()` を呼ぶ（早期リターンで呼ばないと古い線が残る）。
   `@MDC` を参照する Custom RAM がある場合のみ `recomputeCustomRAMs()` を追従実行する
   （ファイル読込時は Custom RAM 生成 → `computeDriveIndex` の順のため、これが無いと
   読込直後だけ全NaNになる）。

6. **入力補完に `@` を含める** (`app.js`): `getWordAtCursor` の正規表現が語頭の `@` を
   拾わないため `@M` と打っても候補が出なかった（`@Integral_Fuel` など既存の Custom RAM も
   同様の既存の穴）。語頭の `@` を許可し、サジェストの種別ラベルに `Mode` を追加。

7. **名前衝突時のメッセージ**: Custom RAM に `MDC` と名付けると自動生成の `@MDC` と衝突するが、
   「既に存在します」だけでは理由が分からないため専用メッセージを出す。

### 使い方

モードを判別すると、チャンネル一覧に `@MDC`（単位 km/h）が自動で現れる。クリックすれば
グラフに重なる。Custom RAM の式からも参照でき、例えば `Actual_Speed - @MDC` で追従誤差の
波形が得られる。

### 検証

- `npm test` 全7本パス（`shortName` の一意性・式パーサ安全性、`cycleChannelName` の
  記号除去とフォールバックのテストを追加）。
- ブラウザ実機で確認: 前後120秒の余分データ付きMDCログで `@MDC` が実測と最大0.37km/h以内で
  重なる（整合が効いている）／サイクル窓外は線が途切れる／`Actual_Speed - @MDC` が計算できる／
  モードをWLTCに切替えると `@WLTC3b_3` へ入れ替わる／目標にCSV列を指定すると消える／
  余分データ量の違うサブファイルが自分の整合位置（start 40.4 対 メイン120.3）で線を持つ／
  `NEDC_sample_A.trn` で `@NEDC` が出る（デグレなし）／コンソールエラーなし。
