...
## 2026-06-26: IWR計算を法規VBA手順に合わせて修正（Claude Code実施）

### 背景
ドライビングインデックスの **IWR（慣性仕事率）** が、法規で使われるVBA計算や他ツールと一致しない。
原因は加速度の出し方が単純な前進差分だったため。法規手順（平滑化＋中心差分）に合わせた。

### 変更（`drive-index-utils.js`）
1. **加速度算出を法規パイプラインに変更**
   - 10Hz補間 → 速度に **5点移動平均×2回** → **0.03 m/s 未満をゼロ化** →
     **中心差分**で加速度（両端は前進/後退差分）→ **正の慣性仕事のみ積算**
   - 移動平均の端処理は「**先頭2点・末尾2点は素通し**、完全な5点窓のある内部のみ平均」
   - 速度の前処理（平滑化＋ゼロ化）は **IWRだけでなく ASCR/DR/ER にも共通適用**
     （`preprocessSpeed()`）。RMSSE は追従誤差指標なので前処理前の生トレースで計算
   - 新規ヘルパー `movingAverage5()` / `preprocessSpeed()` / `inertialWork()`（テスト用にexport）
2. **WOT（アクセル開度≥95%）区間の扱い**
   - WOT区間は実測ではなく **目標車速** を使ってIWRを計算（`WOT_THROTTLE_PCT=95`）
   - `computeMetrics({ ..., throttle })` でアクセル開度配列を受け取る
   - EER は ER/DR から導出（式は据え置き）

### 変更（`app.js`）
- チャンネル解決に **アクセル開度** を追加（`di.channels.throttle`）。
  自動検出（throttle/アクセル/開度 系、「加速度」は除外）＋モーダルで手動選択可能。

### テスト（`tests/drive-index-utils.test.js`）
- トートロジーだったIWRテストを、不変条件ベースに刷新
  （`movingAverage5`の手計算値、定速・微速のゼロ性、目標＝実測でIWR=0、全区間WOTでIWR=0 など）
