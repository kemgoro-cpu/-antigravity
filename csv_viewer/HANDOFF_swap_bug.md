...
## 保留中だった別タスク → **解消済み（2026-07-27）**

WLTC(Extra-Highなし) と MDC のサイクル判別衝突（共に1477秒で長さ判別が決着しない論点）は
**解決済み**。MDCの確定版トレースを入手して内蔵モードへ復帰させ、`detectCycle` に
最高車速タイブレーク（＋決め手が無い場合の波形照合フォールバック）を実装した。
詳細は `CHANGELOG.md` の「2026-07-27: MDC（Malaysian Driving Cycle）を内蔵走行モードに追加」を参照。
本バグとは独立の変更。
